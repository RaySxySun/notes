...
start(){
	...
	${JAVA_EXEC} -server -Dproc_name=${COMPONENT_NAME} -Dfile.encoding=UTF-8 -Ddubbo.properties.file=${COMPONENT_CONF}/dubbo.properties ${JVM_ARGS} \
	             -cp $CLASSPATH com.alibaba.dubbo.container.Main 
	#${JAVA_EXEC} -server -Dproc_name=${COMPONENT_NAME} -Dfile.encoding=UTF-8 -Ddubbo.properties.file=${COMPONENT_CONF}/dubbo.properties ${JVM_ARGS} \
	#             -cp $CLASSPATH com.alibaba.dubbo.container.Main
...
debug() {
	...
	${JAVA_EXEC} -server -agentlib:jdwp=transport=dt_socket,server=y,suspend=y,address=8787 -Dproc_name=${COMPONENT_NAME} -Dfile.encoding=UTF-8 -Ddubbo.properties.file=${COMPONENT_CONF}/dubbo.properties ${JVM_ARGS} \
	             -cp $CLASSPATH com.alibaba.dubbo.container.Main 
	#${JAVA_EXEC} -server -Dproc_name=${COMPONENT_NAME} -Dfile.encoding=UTF-8 -Ddubbo.properties.file=${COMPONENT_CONF}/dubbo.properties ${JVM_ARGS} \
	#             -cp $CLASSPATH com.alibaba.dubbo.container.Main
